...
def check_wakeup(interface):
    ...
    try:
        ...
        logging.info("Wakeup status for {}: {}"
                     .format(interface, wakeup_status))
        ...
        raise FileNotFoundError("The network interface {} does not exist."
                                .format(interface))
...
def s3_or_s5_system(type):
    ...
    if type not in commands:
        raise RuntimeError("Error: type should be s3 or s5(provided: {})"
                           .format(type))
...
def main():
    ...
    if result_enabled is False:
        raise SystemExit("The wake on lan of {} is disabled!"
